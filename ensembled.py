...
#    model.save(f'models/model{n+1}(800).h5')
...
#保存结果
df = pd.DataFrame({'predict':[fine_label_names[i] for i in result],
                   'truth':[fine_label_names[i] for i in test_label]})
df.to_excel(f'ensemble_prediction_{n_estimators}.xlsx',index = False)
